...
export default function PromptGenerator() {
  ...
  const handleFiles = (files) => {
    const allowed = ["application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "text/plain", "text/csv", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"];
    const newFiles = [];
    Array.from(files).forEach((file) => {
      if (file.size > 10 * 1024 * 1024) return; // max 10MB
      const reader = new FileReader();
      reader.onload = (e) => {
        newFiles.push({ name: file.name, size: file.size, type: file.type, base64: e.target.result });
        if (newFiles.length === files.length || newFiles.length === Array.from(files).filter(f => f.size <= 10*1024*1024).length) {
  ...
  const removeArquivo = (idx) => setArquivos(arquivos.filter((_, i) => i !== idx));
...
${infosTransferencia.map((info) => `        - ${info}`).join("\n")}
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      arquivos: arquivos.map(a => ({ name: a.name, size: a.size, type: a.type, base64: a.base64 })),
      promptXML: prompt,
  ...
  const renderConhecimento = () => (
    <>
      <Card title="Base de Conhecimento">
        ...
        <FG><Label sub="Arraste arquivos ou clique para anexar (PDF, DOC, TXT, CSV — máx. 10MB cada)">Anexar Arquivos</Label>
          <div
            onDragOver={(e) => { e.preventDefault(); setDragOver(true); }}
            onDragLeave={() => setDragOver(false)}
            onDrop={(e) => { e.preventDefault(); setDragOver(false); handleFiles(e.dataTransfer.files); }}
            onClick={() => { const inp = document.createElement("input"); inp.type = "file"; inp.multiple = true; inp.accept = ".pdf,.doc,.docx,.txt,.csv,.xls,.xlsx"; inp.onchange = (ev) => handleFiles(ev.target.files); inp.click(); }}
            style={{
              border: `2px dashed ${dragOver ? C.accent : C.inBor}`,
              borderRadius: 12, padding: "28px 20px", textAlign: "center",
              cursor: "pointer", transition: "all .2s",
              background: dragOver ? C.accentLt : "#fff",
            }}
          >
            <div style={{ fontSize: "2rem", marginBottom: 8 }}>📁</div>
            <p style={{ color: dragOver ? C.accent : C.txtSec, fontWeight: 600, fontSize: "0.88rem", margin: "0 0 4px" }}>
              {dragOver ? "Solte os arquivos aqui" : "Arraste arquivos aqui ou clique para selecionar"}
            </p>
            <p style={{ color: C.txtSec, fontSize: "0.75rem", margin: 0 }}>PDF, DOC, DOCX, TXT, CSV, XLS, XLSX</p>
          </div>
              ...
              {arquivos.map((arq, i) => (
                <div key={i} style={{
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <div>
                      <p style={{ margin: 0, fontSize: "0.84rem", fontWeight: 600, color: C.txt }}>{arq.name}</p>
                      <p style={{ margin: 0, fontSize: "0.72rem", color: C.txtSec }}>{formatFileSize(arq.size)}</p>
                    </div>
                  </div>
                  <button onClick={(e) => { e.stopPropagation(); removeArquivo(i); }} style={{
                    background: C.dangerLt, border: "none", borderRadius: 8,
                    color: C.danger, cursor: "pointer", padding: "6px 10px", fontWeight: 600, fontSize: "0.82rem",
                  }}>✕</button>
                </div>
              ))}
            </div>
  ...
  const renderRevisao = () => (
    <>
      ...
      <Card title="📚 Conhecimento">
        ...
        <ReviewRow label="Anexo PDF" value={arquivos.length > 0 ? `${arquivos.length} arquivo(s): ${arquivos.map(a => a.name).join(", ")}` : "Nenhum"} />
